...
execute "install_mongo_hacker" do
  command [
    ...
    "make",
    "ln -f mongo_hacker.js #{ENV['HOME']}/.mongorc.js",
    "chown root:    #{ENV['HOME']}/.mongorc.js",
    "rm -rf /tmp/{mongo-hacker-master,master.zip}"
  ].join(" && ")
  not_if { ::File.exists?("#{ENV['HOME']}/.mongorc.js") }
end

execute "clean_up_vagrant_omnibus" do
  command "rm -f #{ENV['HOME']}/install.sh"
end

cookbook_file "public_ip" do
  path  "#{ENV['HOME']}/public_ip"
  owner "root"
  group "root"
  mode  "0755"
end
